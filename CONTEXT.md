...
**深度研究（deep research）**:
面向复杂主题的多源证据收集、交叉验证并综合成报告的一次性编排服务，由 `deep_research` 工具（模型按需触发）或 `/deep-research` 命令（意图入口）触发。
...
**意图入口（intent entry）**:
`/deep-research` 命令的触发语义：命令只表达研究意图，由主 agent 承接并决定澄清后调用 `deep_research` 工具，而非命令自行执行研究。
_Avoid_: 命令直达（命令直接执行编排的旧语义）

**澄清（clarification）**:
主 agent 在启动 run 之前、仅在研究用途或范围不足时向用户提出 1–2 个关键问题的动作；信息足够则直接开始。
_Avoid_: 访谈（暗示固定的人机问答环节；澄清是判断性的兜底动作，非流程阶段、非代理角色）
