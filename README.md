...
> ⚠️ **深度 / 轻量研究边界**：轻量主题调研、文档/API 事实收集、把阅读工作委托给后台代理这类请求**不要**路由到本插件的 `deep_research` 工具（按轻量方式直接处理即可）。`deep_research` 只应响应明确的**深度**研究意图（`/deep-research` 命令、或「深度调研／深度研究／系统调研报告」）。插件已把 `deep_research` 工具描述限定为深度研究、并显式排除轻量调研场景；轻量方式由宿主环境提供，本插件不绑定、也不降级到任何外部技能——引擎等前置条件不满足时只会明确报错并给出修复步骤。
